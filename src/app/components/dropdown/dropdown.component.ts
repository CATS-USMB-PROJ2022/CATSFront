...
export class DropdownComponent implements OnInit {
  ...
  constructor(private CallService: CallService, public cookieService: CookieService) {
    this.rubIdCaisse = [0];
    this.caisse = [""];
  ...
  private initDataCalls(/*caisse: number, */date_start: Date, date_end: Date, time_start: Time, time_end: Time, gt: string[] = []) {
    ...
    this.CallService.postNumberCall(Number(this.nbrCaisse), date_start, date_end, time_start, time_end, gt).subscribe(data => {
      console.log(data);
